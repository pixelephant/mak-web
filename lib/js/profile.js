...
$(function() {
	...
	$("#brand2").change(function(){
		$.post("lib/php/profile.php",{
			"action" : 'brandType',
			"brand" : $("#brand2 option:selected").val()
		},function(resp){
			if(resp != 'sikertelen'){
	...
	//Céges irányítószám - levelezési cím 
	$("#coZipMailing").keyup(function(){
		if($(this).val().length === 4 && $("#coZipMailing").valid()){
			$.post("lib/php/ir.php",{
				"num" : $(this).val()
			},function(resp){
				if(resp == ''){
	...
	//Magán irányítószám - levelezési cím
	$("#natZipMailing").keyup(function(){
		if($(this).val().length === 4 && $("#natZipMailing").valid()){
			$.post("lib/php/ir.php",{
				"num" : $(this).val()
			},function(resp){
				if(resp == ''){
	...
	$("#registerform").validate({
		...
		submitHandler: function(form) {
			...
			editFormData = $("#registerform").serialize();
			
			$.post("lib/php/profile.php",{
				"editformData" : editFormData
			},function(resp){
				if(resp == "sikeres"){
					$("#response").html("<p class='success'>Sikeres adatmódosítás.</p>");
					return false;
				}
				else{
					$("#response").html("<p class='error'>Hiba történt. Kérem próbálja újra.</p>");
					return false;
				}
			});
