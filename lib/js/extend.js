...
jQuery.validator.addMethod("platehu", function(value, element) {
	return this.optional(element) || /[A-Za-z]{3}[0-9]{3}/i.test(value);
...
$(function() {
	...
	$("#extendButton").click(function(e){
		...
		e.preventDefault();
		var cur = $(".full").data("currentlevel");
		
		$("#actiontype").html(" - taghosszabbításhoz");
		$("#uzenet").val("Taghosszabítás: ");
	...
	$("#levelButton").click(function(e){
		...
		$("#actiontype").html(" - magasabb tagsági szint választáshoz");
		$("#uzenet").val("Tagsági szint váltás: ");
		...
		submitHandler: function(form) {
			...
			$.post("lib/php/extend.php",{
				...
				if(resp == "sikeres"){
					...
					$("#registerform").append("<label class='error'>Hiba.</label>");
	...
	$("#memberform input[type='radio']").click(function(){
		var cp = $(".full").data("currentprice");
		var p = $(this).data("price");
		
		$(".sum").html(addSpaces(parseInt(p)) + " Ft/év");
		$("#osszeg").val(parseInt(p));
		$("#uzenet").val($("#uzenet").val() + $("#osszeg").val());
	});
	
	$("#memberform").validate({
		submitHandler: function(form) {
		
			var terms = '<label for="terms4">Felhatalmazom a Magyar Autóklubot, hogy a tagsági kártyához kapcsolódó csoportos közlekedési és balesetbiztosítási jogviszony biztosító általi nyilvántartásához szükséges személyes adataimat a Generali-Providencia Biztosító részére átadja. Mint biztosított, hozzájárulok ahhoz, hogy a Biztosító a biztosítási szolgáltatás nyújtására vele szerződéses viszonyban álló jogalanynak a szerződéssel kapcsolatos, biztosítási titkot képező személyes adataimat, valamint a biztosítási fedezet fennállását igazoló adatokat a szolgáltatási igény elbírálása céljából, az elbírálás időtartamára átadja, feltéve, hogy e jogalany véleményének beszerzése a szolgáltatási igény eldöntése szempontjából elkerülhetetlenül szükséges.</label><input class="required" type="checkbox" name="terms4" id="terms4" />';
			...
			if($("#standardMember").is(":checked")){
				$("#komfortTerms").html(terms);
				
				if($(".full").data("usertype") == 'C'){
					$("#memberform").hide();
					$("#standardform").show();
	...
	$("#paymentform").validate({
		submitHandler: function(form) {
			...
			$.post("lib/php/extend.php",{
				...
				if(resp == "Sikeres"){
					if(action == 'extend'){
						msg = 'Hosszabbítási igényét könyveltük!';
					}else{
						msg = 'Szintváltási igényét könyveltük!';
					}
				}else{
		...
		errorPlacement: function(error, element) {
		     error.prependTo(element.parents(".row"));
		}
	});
...
/*	$("#toStep2").click(function(){
	...
	$("#brand").change(function(){
		$.post("lib/php/profile.php",{
			"action" : 'brandType',
			"brand" : $("#brand option:selected").val()
		},function(resp){
			if(resp != 'sikertelen'){
				
				var values = $.parseJSON(resp);
				
				var option = '';
				
	            $.each(values, function() {
	            	option += '<option value="' + this.gyartmany_sap_kod + '">' + this.tipus + '</option>';
	            });
	...
	$("#emailButton").click(function(e){
		...
		$.post("lib/php/extend.php",{
			...
			if(resp == "sikeres"){
				$("#msg").html("Ügyintézőink felveszik Önnel a kapcsolatot!");
			}else{
				$("#msg").html("Ismételje meg az e-mail küldést!");
			}
		});
	});
