...
$(function() {
			...
			if($("#cardNum").val().length === 11 && $("#oldEmail").valid()){
				$.post("lib/php/reg.php",{
					...
					if(resp != null && resp != 'null'){
						...
						//console.log(response.allando_irsz);
						$("#zip").val(response.allando_irsz);
						$("#add").val(response.allando_kozterulet + " " +response.allando_hazszam);
						$("#city").val(response.allando_helyseg);
						//$("#add").val(response.allando_kozterulet);
						...
						$("#natName").val(response.elonev+" "+response.vezeteknev+" "+response.keresztnev);
						$("#natDate").val(response.szuletesi_datum);
						$("#natMother").val(response.anyja_neve);
						$("#natAddress").val(response.allando_kozterulet);
						$("#natPost").val(response.levelezesi_kozterulet);
	...
	$("#registerform").validate({
		...
		submitHandler: function(form) {
			...
			memberRadio = $("input[name='group1']:checked").attr("id");
			registerRadio = $("input[name='group2']:checked").val("id");
			
			registerFormDate.push({"memberRadio",memberRadio},{"registerRadio",registerRadio});
	...
	$("#memberform").validate({
		submitHandler: function(form) {
			...
			membershipRadio = $("input[name='membership']:checked").val("id");
			
			memberFormData.push({"membershipRadio",membershipRadio});
			
			if($("#webMember").is(":checked")){
				$.post("lib/php/register.php",{
					"formData" : registerFormData,
					"memberData" : memberFormData
				},function(resp){
					if(resp == "sikeres"){
						window.location = "registersucces.php";
					}
					else{
						$("#memberform").append("<label class='error'>Hiba.</label>")
					}
				});
			}
			else{
	...
	$("#standardform").validate({
		submitHandler: function(form) {
			...
			var plateTypeRadio = $("input[name='platetype']:checked").val("id");
			
			standardFormData.push({"plateTypeRadio",plateTypeRadio});
	...
	$("#paymentform").validate({
		submitHandler: function(form) {
			...
			var paymentmethodRadio = $("input[name='platetype']:checked").val("id");
			
			paymentFormData.push({"paymentmethodRadio",paymentmethodRadio});
