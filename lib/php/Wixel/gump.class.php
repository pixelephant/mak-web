...
{	
	 ...
	 * @return string
	 */
	protected static function filter_sanitize_numbers_only($value, $params = NULL)
	{
		$var = filter_var($value, FILTER_SANITIZE_NUMBER_INT);
	...
	protected static function validate_alpha($field, $input, $param = NULL)
	{
		...
		//$input[$field] = iconv("UTF-8", "ISO-8859-1", $input[$field]);
		
		print_r($input[$field]);
		
		if(!preg_match("/^([a-ű])+$/i", $input[$field]) !== FALSE)
		{
		
			//$input[$field] = utf8_encode($input[$field]);
	...
	{	
		...
		if(!preg_match("/^([a-ű0-9])+$/i", $input[$field]) !== FALSE)
		{
			return array(
				'field' => $field,
				'value' => $input[$field],
