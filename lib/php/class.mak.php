...
class mak extends db{
	...
	public function get_tartalom($cond='',$col='',$join=''){
		...
		if($col == ''){
			...
			$col .= 'mak_kategoria.email AS email,mak_kategoria.telefon AS telefon,mak_kategoria.kategoria_nev AS kategoria_nev,mak_kategoria.azonosito AS azonosito,mak_almenu.url AS url,mak_almenu.almenu AS almenu,mak_almenu.title AS title,mak_almenu.description AS description,mak_almenu.keywords AS keywords';
		}
	...
	public function get_h1_urlbol($url){
		...
		$table = 'mak_almenu';
		$col = 'mak_almenu.almenu AS almenu';
	...
	public function render_aloldal_section($aloldal){
	...
	public function render_aloldal_bal_menu($aloldal){
		...
		$html .= '<img src="' . $this->_imageDir . $aloldalak[0]['azonosito'] . '.jpg" alt="' . $aloldalak[0]['kategoria_nev'] . '" />';
